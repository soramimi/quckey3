...
#if MOUSE_ENABLED
PROGMEM static const uint8_t mouse_hid_report_desc[] = {
...
static volatile uint8_t suspend_state = 0;
...
static inline void release_rx()
{
	UEINTX = 0x6b;
}

static inline void release_tx()
{
	UEINTX = 0x3a;
}

int8_t usb_send(uint8_t ep, uint8_t const *ptr, uint8_t len)
{
...
static inline void usb_stall()
{
	UECONX = (1 << STALLRQ) | (1 << EPEN);
}

static inline void usb_gen_vect()
{
	uint8_t udint = UDINT;
	if (udint & (1 << EORSTI)) {
		UENUM = 0;
...
//		usb_flush(TX_EP);
	...
	if (udint & (1 << WAKEUPI)) {
		...
		suspend_state = (suspend_state & (1 << SUSPI)) | (1 << WAKEUPI);
	} else if (udint & (1 << WAKEUPI)) {
		UDIEN = (UDIEN & ~(1 << SUSPE)) | (1 << WAKEUPE);
		suspend_state = (suspend_state & (1 << WAKEUPI)) | (1 << SUSPI);
	}
	UDINT = 0;
}

static inline void usb_com_vec()
{
	UENUM = 0;
